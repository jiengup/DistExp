...
public:
  rpc_client(bool if_fast_transmition = false)
    : socket_(ios_), work_(ios_), deadline_(ios_), body_(INIT_BUF_SIZE) {
      // 设置不延迟发送选项
      enable_fast_transmission = if_fast_transmition;
...
private:
  ...
  void reset_socket() {
    ...
    if (!socket_.is_open()) {
      ...
      if(enable_fast_transmission){
        asio::ip::tcp::no_delay _option(true);
        socket_.set_option(_option);
  ...
  bool enable_fast_transmission = false;
