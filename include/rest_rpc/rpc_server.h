...
public:
  ...
  void set_fast_transmition(bool if_fast_transmition) {
    enable_fast_transmition = if_fast_transmition;
  }
...
private:
...
#endif
        if(enable_fast_transmition){
          asio::ip::tcp::no_delay noDelayOption(true);
          conn_->socket().set_option(noDelayOption);
          // 设置 quick_ack 选项
          // 注意：Boost.Asio库本身并不直接支持设置quick_ack选项，你可能需要使用原生socket API来设置这个选项
          // 这里只是一个示例，可能需要根据你的实际情况进行修改
          int quickAck = 1;
          setsockopt(conn_->socket().native_handle(), IPPROTO_TCP, TCP_QUICKACK, &quickAck, sizeof(quickAck));
        }
  ...
  bool enable_fast_transmition = false;
